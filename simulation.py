...
def run_all_cases(payload: dict) -> dict:
    ...
    if cas_extremes:
        ...
        payload_chaud = appliquer_cas_extreme(payload, f_masse=1.0, f_qv=1.0 + f_ch, f_vent=1.0 - f_ch, f_transp=0.1)
        chaud = run_simulation_core(payload_chaud, label_cas="Cas chaud")

        # f_transp=1.0 : animal froid → métabolisme réduit mais transpiration intacte
        payload_froid = appliquer_cas_extreme(payload, f_masse=1.0, f_qv=1.0 - f_fr, f_vent=1.0 + f_fr, f_transp=1.0)
